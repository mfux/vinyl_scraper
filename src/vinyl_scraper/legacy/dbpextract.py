...
from yt_mp3_downloader import download
...
def get_html(page):
    ...
    while counter < page:
        ...
        findID = re.findall(r'post-body-(.*)\' itemprop', lines)  # This retrieves each post's unique ID-number.
    ...
    with open(tfn, 'r') as f:
        ...
        for info in infos:
            ...
            if Path('mp3/' + info["Description"].replace("/", " ") + '.mp3').exists():
                try:
                    write_info(info, 'mp3/' + info["Description"].replace("/", " ") + '.mp3')
                except (IOError, AttributeError, IndexError):
                    continue
                continue
            try:
                yt_info = download(info["Link"], 'mp3/' + info["Description"].replace("/", " "))
            except:
                continue
